...
// Check if a map cell is edible by a given player
static bool is_edible_by(char cell, bool is_player1)
{
    if (cell == 'F') return true;
    if (is_player1 && cell == MAP_CHAR_P1_DROP) return true;
    if (!is_player1 && cell == MAP_CHAR_P2_DROP) return true;
    return false;
}
...
void init_battle_game(GameState *game, AIDifficulty difficulty)
{
    ...
    game->p1_drop_requested = false;
    game->p2_drop_requested = false;
                            ...
                            bool is_player1)
{
    ...
    // Opponent's dropped block → obstacle
    if ((is_player1  && cell == MAP_CHAR_P2_DROP) ||
        (!is_player1 && cell == MAP_CHAR_P1_DROP))
    {
        if (out_reason) *out_reason = DEATH_OBSTACLE;
        return false;
    }
...
bool can_start_in_direction(GameState *game, Direction dir)
{
    ...
    return can_enter_cell(game, next, false, game->snake, game->length, &dummy, true);
}
...
void move_snake(GameState *game)
{
    ...
    if (!can_enter_cell(game, next, will_grow, game->snake, game->length, &reason, true))
    {
        game->game_over = true;
...
// Apply a single snake move. Returns true only when real food 'F' was eaten
// (so the caller knows to place a new food item).
// When drop_requested && length > 3, a tail segment is dropped as a block.
static bool apply_snake_move(GameState *game, Point *body, int *length,
                              Direction dir, int *score, int *food_eaten,
                              int *pending_growth,
                              char head_char, char body_char,
                              bool *drop_requested, char drop_char)
{
    Point old_head = body[0];
    Point next = next_head(old_head, dir);
    char next_cell = game->map[next.row][next.col];

    // Determine own dropped-block character for this snake
    char own_drop = (head_char == 'H') ? MAP_CHAR_P1_DROP : MAP_CHAR_P2_DROP;
    bool will_eat_food     = (next_cell == 'F');
    bool will_eat_own_drop = (next_cell == own_drop);
    bool will_eat          = will_eat_food || will_eat_own_drop;
    bool auto_grow = ((game->move_count + 1) % game->grow_step) == 0;

    // ── Handle drop request ──────────────────────────────
    bool actually_dropped = false;
    Point vacated_pos = {0, 0};   // cell that falls out of body after drop+shift
    bool has_vacated = false;
    if (*drop_requested && *length > 3)
    {
        Point old_tail = body[*length - 1];

        if (will_eat)
        {
            // Eating + dropping simultaneously:
            //   Food growth offsets the drop → tail stays, length unchanged.
            //   pending_growth decreases by 1 (floored at 0).
            if (*pending_growth > 0)
                (*pending_growth)--;
            // Block both tail removal and length++ by marking as dropped.
            // will_eat remains true for score/food_eaten below.
            actually_dropped = true;
        }
        else
        {
            // Normal drop: remove tail, place dropped block on map.
            // The body shift will leave a stale 'B' at the cell that was
            // body[*length-2]; save it so we can clear it after the shift.
            game->map[old_tail.row][old_tail.col] = drop_char;
            vacated_pos = body[*length - 2];   // second-to-last, about to be vacated
            has_vacated = true;
            (*length)--;
            actually_dropped = true;

            // Convert auto-grow into pending so it isn't lost
            if (auto_grow)
            {
                (*pending_growth)++;
                auto_grow = false;
            }
            // Consume one pending credit if available
            if (*pending_growth > 0)
                (*pending_growth)--;
        }
    ...
    // Remove tail (when not already dropped and not growing)
    if (!actually_dropped && !effective_grow)
    {
        Point tail = body[*length - 1];
        game->map[tail.row][tail.col] = '.';
    }
    else if (!actually_dropped && effective_grow && *length < MAX_SNAKE_LEN)
    {
        (*length)++;
    }
    ...
    // Clear the cell that fell out of the body array after a drop
    if (has_vacated)
        game->map[vacated_pos.row][vacated_pos.col] = '.';
    ...
    return will_eat_food;  // only place_food for real 'F'
}
...
void move_battle_snakes(GameState *game, bool move_p1, bool move_p2)
{
    ...
    // Compute AI direction (only when AI moves)
    Direction ai_dir = game->ai_direction;
    if (move_p2 && game->ai_is_alive)
    {
        ai_dir = ai_choose_direction(game, game->ai_difficulty);
        game->ai_direction = ai_dir;
    }

    // Compute next head positions (current position when not moving)
    Point player_next = move_p1 ? next_head(game->snake[0], game->direction)
                                : game->snake[0];
    Point ai_next = (move_p2 && game->ai_is_alive)
                        ? next_head(game->ai_snake[0], ai_dir)
                        : game->ai_snake[0];
    ...
    bool player_will_eat = move_p1 && (player_cell == 'F' || player_cell == MAP_CHAR_P1_DROP);
    bool ai_will_eat = move_p2 && (ai_cell == 'F' || ai_cell == MAP_CHAR_P2_DROP);
    bool auto_grow = ((game->move_count + 1) % game->grow_step) == 0;
    bool player_will_grow = move_p1 && (player_will_eat || auto_grow);
    bool ai_will_grow = move_p2 && (ai_will_eat || auto_grow);
    ...
    if (is_same_point(player_next, ai_next))
    {
        ...
        {
            // Only P1 moved into stationary AI head
            player_dies = true;
            player_reason = DEATH_HEAD_ON;
        }
        else if (move_p2)
        {
            // Only AI moved into stationary player head
            ai_dies = true;
            ai_reason = DEATH_HEAD_ON;
        }
    }
    else
    {
        ...
        // 5b. Player hits P2/AI's dropped block → obstacle
        if (player_cell == MAP_CHAR_P2_DROP)
        {
            player_dies = true;
            player_reason = DEATH_OBSTACLE;
        }

        // 5c. AI hits P1's dropped block → obstacle
        if (ai_cell == MAP_CHAR_P1_DROP)
        {
            ai_dies = true;
            ai_reason = DEATH_OBSTACLE;
        }
    ...
    // Apply player move (only when P1 should move)
    if (move_p1 && !player_dies)
    {
        bool ate = apply_snake_move(game, game->snake, &game->length,
                                     game->direction, &game->score, &game->food_eaten,
                                     &game->pending_growth,
                                     'H', 'B',
                                     &game->p1_drop_requested, MAP_CHAR_P1_DROP);
        if (ate)
            place_food(game);
    }

    // Apply AI move (only when P2/AI should move)
    if (move_p2 && !ai_dies && game->ai_is_alive)
    {
...
static bool p2_can_start_in_direction(GameState *game, Direction dir)
{
    ...
    DeathReason dummy = DEATH_NONE;
    return can_enter_cell(game, next, false, game->ai_snake, game->ai_length, &dummy, false);
}
...
void move_pvp_snakes(GameState *game, bool move_p1, bool move_p2)
{
    ...
    // Compute next head positions (current position when not moving)
    Point p1_next = move_p1 ? next_head(game->snake[0], p1_dir) : game->snake[0];
    Point p2_next = move_p2 ? next_head(game->ai_snake[0], p2_dir) : game->ai_snake[0];

    // Check what's at each next cell
    char p1_cell = game->map[p1_next.row][p1_next.col];
    char p2_cell = game->map[p2_next.row][p2_next.col];

    bool p1_will_eat = move_p1 && (p1_cell == 'F' || p1_cell == MAP_CHAR_P1_DROP);
    bool p2_will_eat = move_p2 && (p2_cell == 'F' || p2_cell == MAP_CHAR_P2_DROP);
    bool auto_grow = ((game->move_count + 1) % game->grow_step) == 0;
    bool p1_will_grow = move_p1 && (p1_will_eat || auto_grow);
    bool p2_will_grow = move_p2 && (p2_will_eat || auto_grow);
    ...
    else
    {
        ...
        // 5b. P1 hits P2's dropped block → obstacle
        if (p1_cell == MAP_CHAR_P2_DROP)
        {
            p1_dies = true;
            p1_reason = DEATH_OBSTACLE;
        }

        // 5c. P2 hits P1's dropped block → obstacle
        if (p2_cell == MAP_CHAR_P1_DROP)
        {
            p2_dies = true;
            p2_reason = DEATH_OBSTACLE;
        }
    ...
    if (p1_dies)
    {
        ...
        if (move_p2 && !p2_dies && game->ai_is_alive)
        {
            apply_snake_move(game, game->ai_snake, &game->ai_length,
