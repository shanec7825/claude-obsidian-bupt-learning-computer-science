...
// ──── Resources ────────────────────────────────────────────
const Color COL_DROP_P1       = { 46, 204, 113, 255};  // P1-dropped block (green)
const Color COL_DROP_P2       = { 52, 152, 219, 255};  // P2-dropped block (blue)
...
static Color cell_render_color(const GameState *game, int r, int c, char cell)
{
    ...
    case MAP_CHAR_P1_DROP: return COL_DROP_P1;
    case MAP_CHAR_P2_DROP: return COL_DROP_P2;
...
static void draw_cell(const GameState *game, int r, int c, char cell)
{
    ...
    // Dropped tail block: rounded square with "×" mark
    if (cell == MAP_CHAR_P1_DROP || cell == MAP_CHAR_P2_DROP)
    {
        Color drop_color = (cell == MAP_CHAR_P1_DROP) ? COL_DROP_P1 : COL_DROP_P2;
        float margin = 3.0f;
        float w = (float)CELL_SIZE - margin * 2.0f;
        DrawRectangleRounded(
            {(float)left + margin, (float)top + margin, w, w},
            0.25f, 6, drop_color);
        // "×" cross mark
        int cx = left + CELL_SIZE / 2;
        int cy = top  + CELL_SIZE / 2;
        int cross_r = CELL_SIZE / 5;
        Color cross_col = {255, 255, 255, 200};
        DrawLine(cx - cross_r, cy - cross_r, cx + cross_r, cy + cross_r, cross_col);
        DrawLine(cx - cross_r, cy + cross_r, cx + cross_r, cy - cross_r, cross_col);
    }
...
void draw_panel(const GameState *game)
{
    ...
    if (game->mode == MODE_PVP)
    {
        ...
        DrawTextF("F：P1加速   R_Shift：P2加速",   px, 600, 16, COL_TEXT_DIM);
        DrawTextF("G：丢弃(P1)   L：丢弃(P2)",       px, 630, 16, COL_TEXT_DIM);
        DrawTextF("Esc：返回菜单",                   px, 660, 16, COL_TEXT_DIM);
        DrawTextF("模式：双人对战 (1P vs 2P)",        px, 720, 16, COL_TEXT_DIM);
    }
    else if (game->mode == MODE_BATTLE)
    {
        ...
        DrawTextF("空格：暂停   R：重新开始", px, 570, 16, COL_TEXT_DIM);
        DrawTextF("F：加速",                   px, 600, 16, COL_TEXT_DIM);
        DrawTextF("G：丢弃方块",               px, 630, 16, COL_TEXT_DIM);
        DrawTextF("Esc：返回菜单",             px, 660, 16, COL_TEXT_DIM);
        ...
        DrawTextF(diff_line, px, 690, 16, COL_TEXT_DIM);

        DrawTextF("模式：人机对战 (玩家 vs AI)", px, 720, 16, COL_TEXT_DIM);
    }
    else
    {
